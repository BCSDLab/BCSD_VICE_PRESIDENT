...
- 입금확인증 PDF 첨부 (`--transfer-proof`): poppler (`pdftoppm`, `pdftotext`) 시스템 패키지 필요
...
```bash
python main.py <시작기간> <종료기간> [--transfer-proof PDF]
# 예시
python main.py 2025-11 2026-02
python main.py 2025-11 2026-02 --transfer-proof report-1.pdf
```
...
| 옵션 | 설명 |
|---|---|
| `--transfer-proof PDF` | 이체 내역 입금확인증 PDF 경로. 지정 시 각 페이지를 이미지로 변환해 해당 지출 레코드의 증빙 표 앞에 첨부. `pdftoppm` / `pdftotext` (poppler) 필요. |

입금확인증은 `거래일시 + (입금금액 + 수수료) = abs(입/출)` 기준으로 레코드에 매칭되며, PDF의 실제 수수료 값으로 이체 수수료 표기도 자동 보정됩니다.
...
│       └── transfer_proof.py       # 입금확인증 PDF 파싱 + 장부 매칭
├── templates/
